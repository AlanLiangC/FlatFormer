...
class FlashAttention(nn.MultiheadAttention):
    def forward(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        ...
        x = flash_attn_varlen_qkvpacked_func(qkv, cu_seqlens, num_tokens, 0)
...
class FlattenedWindowMapping(nn.Module):
    ...
    def forward(self, coords: torch.Tensor, batch_size: int) -> Dict[str, torch.Tensor]:
        ...
        _, num_per_batch = torch.unique(coords[:, 0], sorted=False, return_counts=True) # [5796, 7549, 3984, 6293]
        batch_start_indices = F.pad(torch.cumsum(num_per_batch, dim=0), (1, 0)) # [0,  5796, 13345, 17329, 23622]
        num_per_batch_p = ( # [5904, 7632, 4032, 6336]
            torch.div(
        ...
        batch_start_indices_p = F.pad(torch.cumsum(num_per_batch_p, dim=0), (1, 0)) # [0,  5904, 13536, 17568, 23904]
...
class PositionalEmbedding(nn.Module):
    ...
    def forward(self, coors, dtype):
        ...
        pex = x[:, None] / inv_freq()[None, :] # [23905, 64]
        pey = y[:, None] / inv_freq()[None, :] # [23905, 64]

        # [num_tokens, pos_length]
        pex = torch.stack([pex[:, ::2].sin(), pex[:, 1::2].cos()], dim=-1).flatten(1) # [23905, 64]
        pey = torch.stack([pey[:, ::2].sin(), pey[:, 1::2].cos()], dim=-1).flatten(1)
        pe = torch.cat([pex, pey], dim=-1).to(dtype) # [23905, 128]
    ...
    def inv_freq(self):
        ndim = 2
        pos_length = (self.feat_dim // (ndim * 2)) * 2 # 64
...
class FlatFormerEncoder(BaseModule):
    def __init__(
        self,
        in_channels=128,
        num_heads=8,
    ...
    def forward(self, x, coords, batch_size):
        pe = self.embedding(coords, x.dtype) # [23905, 128]
